...
1.  What's the `tidyverse` Package? Describe in maximum two sentences below.
    ...
    The tidyverse is a collection of R packages designed for data science that share a common philosophy, grammar, and data structures, including packages like ggplot2, dplyr, tidyr, and readr. It simplifies data manipulation, visualization, and analysis by using consistent syntax and principles
...
2.  What's the [`here` Package](https://here.r-lib.org/articles/here.html)? Why is useful when working with quarto or R Markdown files?
    ...
    The here package helps manage file paths reliably by setting the project’s root directory as the reference point, avoiding hard-coded paths. This is especially useful in Quarto or R Markdown documents, where relative paths can break when files are moved or the working directory changes.
